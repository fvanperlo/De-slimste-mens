...
const ReviewPuzzle: React.FC<ReviewPuzzleProps> = ({ data, onConfirm, onBack }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 bg-[#850000] border border-[#a00000] rounded shadow-2xl">
      <div className="mb-6 flex justify-between items-center border-b border-[#a00000] pb-4">
        <div>
          <h2 className="text-3xl font-bold text-white uppercase tracking-tight">Controleer Puzzel</h2>
          <p className="text-red-100 text-sm">Pas de omschrijvingen aan waar nodig voordat je start.</p>
        </div>
        <button 
          onClick={onBack}
          className="text-red-200 hover:text-white flex items-center gap-2 text-sm uppercase font-bold transition-colors"
        >
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            className="w-full bg-[#650000] border-[#750000] text-white text-lg rounded px-4 py-2 focus:ring-2 focus:ring-white/20 outline-none"
          />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {editableData.groups.map((group, groupIndex) => (
            <div key={group.id} className="bg-[#750000] p-4 rounded border border-[#850000]">
              <div className="mb-4">
                <label className="block text-xs font-bold text-red-200 uppercase mb-1">Begrip {groupIndex + 1}</label>
                <div className="relative">
                  <input
                    type="text"
                    value={group.term}
                    onChange={(e) => handleTermChange(groupIndex, e.target.value)}
                    className="w-full bg-[#600000] border border-[#750000] text-white font-bold rounded px-3 py-2 pr-8 focus:ring-1 focus:ring-white/20 outline-none"
                  />
                  <Pencil className="w-4 h-4 text-red-300 absolute right-3 top-3 pointer-events-none" />
                </div>
              </div>

              <div className="space-y-2">
                <label className="block text-xs font-bold text-red-200 uppercase">Omschrijvingen</label>
                {group.clues.map((clue, clueIndex) => (
                  <input
                    ...
                    className="w-full bg-[#650000] border-b border-[#750000] text-red-100 text-sm px-2 py-2 focus:bg-[#750000] focus:border-red-300 outline-none transition-colors"
                  />
                ))}
              </div>
            </div>
      ...
      <div className="mt-8 pt-6 border-t border-[#a00000] flex justify-end">
        <button
