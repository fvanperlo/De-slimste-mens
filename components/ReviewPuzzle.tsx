...
const ReviewPuzzle: React.FC<ReviewPuzzleProps> = ({ data, onConfirm, onBack }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 bg-[#cc0000] border border-[#ff6666] rounded shadow-2xl">
      <div className="mb-6 flex justify-between items-center border-b border-[#ff6666] pb-4">
        <div>
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            className="w-full bg-[#b30000] border-[#d90000] text-white text-lg rounded px-4 py-2 focus:ring-2 focus:ring-white/20 outline-none"
          />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {editableData.groups.map((group, groupIndex) => (
            <div key={group.id} className="bg-[#d90000] p-4 rounded border border-[#ff6666]">
              <div className="mb-4">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-[#990000] border border-[#ff6666] text-white font-bold rounded px-3 py-2 pr-8 focus:ring-1 focus:ring-white/20 outline-none"
                  />
              ...
              <div className="space-y-2">
                ...
                {group.clues.map((clue, clueIndex) => (
                  <input
                    ...
                    className="w-full bg-[#b30000] border-b border-[#ff6666] text-white text-sm px-2 py-2 focus:bg-[#c60000] focus:border-white outline-none transition-colors"
                  />
                ))}
              </div>
            </div>
      ...
      <div className="mt-8 pt-6 border-t border-[#ff6666] flex justify-end">
        <button
          onClick={() => onConfirm(editableData)}
          className="bg-white text-[#cc0000] hover:bg-gray-100 px-8 py-3 rounded font-bold text-xl uppercase tracking-widest flex items-center gap-3 transition-transform transform hover:scale-[1.02] shadow-xl"
        >
